...
from semantic_ai_washing.director.core.audit import (
    default_provenance as director_default_provenance,
)
...
def test_default_provenance_is_generic_in_labcore(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
...
def test_director_default_provenance_keeps_director_identity(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    (repo / ".git").mkdir(parents=True)

    provenance = director_default_provenance(repo)

    assert provenance["tool"] == "semantic_ai_washing.director"
