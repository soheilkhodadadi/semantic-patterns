...
from semantic_ai_washing.director.core.openai_responses import (
    OpenAIResponsesHTTPError as DirectorOpenAIResponsesHTTPError,
)
from semantic_ai_washing.director.core.openai_responses import (
    extract_response_text as director_extract_response_text,
)
...
def test_extract_response_text_prefers_output_text_and_matches_director() -> None:
    payload = {"output_text": "  hello world  "}
    assert extract_response_text(payload) == "hello world"
    assert director_extract_response_text(payload) == "hello world"
...
def test_call_responses_api_normalizes_http_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert isinstance(exc_info.value, DirectorOpenAIResponsesHTTPError)
