...
from semantic_ai_washing.classification import classify_active_window_preliminary_restartable
...
def test_restartable_classification_writes_outputs_and_progress(
    ...
    def fake_predict(sentences: list[str], runtime_manifest: dict[str, str]):
        return ["Actionable"] * len(sentences), [
            {"Actionable": 0.8, "Speculative": 0.1, "Irrelevant": 0.1}
            for _ in sentences
        ]
    ...
    assert (tmp_path / "classifications" / "year=2021" / "model=binary_relevance_then_as_v1" / "classified_sentences.parquet").exists()
    assert (tmp_path / "classifications" / "year=2022" / "model=binary_relevance_then_as_v1" / "classified_sentences.parquet").exists()
