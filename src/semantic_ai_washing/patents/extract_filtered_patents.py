...
for _, firm in firm_df.iterrows():
    ...
    if firm_hits:
        ...
        sub["cik"] = cik
        sub["name"] = name
        sub["match_rule"] = sub["match_term"].apply(lambda x: "alias_regex" if x in firm_aliases.get(cik, []) else "name_regex")
...
if matched_blocks:
    matched_df = pd.concat(matched_blocks, ignore_index=True).drop_duplicates(subset=["patent_id", "cik"])
else:
    matched_df = pd.DataFrame(columns=["patent_id","cik","name","match_rule"])

print(f"[✓] Firms matched: {match_stats.get('firms_matched',0)} | Firms unmatched: {match_stats.get('firms_unmatched',0)}")
...
patents = pd.read_csv(patent_path, sep="\t", usecols=["patent_id", "patent_date", "patent_title"], dtype={"patent_id": str}, low_memory=False)
abstracts = pd.read_csv(abstract_path, sep="\t", usecols=["patent_id", "patent_abstract"], dtype={"patent_id": str}, low_memory=False)
...
totals = (
    df.groupby(["cik", "name", "year"])
      .size()
      .reset_index(name="patents_total")
)
ai_only = (
    df[df["has_ai"]]
      .groupby(["cik", "name", "year"])
      .size()
      .reset_index(name="patents_ai")
)
...
firm_diag = (
    agg.groupby(["cik","name"], as_index=False)
       .agg(patents_total=("patents_total","sum"),
            patents_ai=("patents_ai","sum"))
)
firm_diag["ai_share_overall"] = firm_diag["patents_ai"] / firm_diag["patents_total"].replace(0, pd.NA)

# Add simple match coverage info
coverage = matched_df.groupby(["cik","name"], as_index=False).size().rename(columns={"size":"matched_patent_rows_premerge"})
...
if not top_ai.empty:
    ...
    for _, r in top_ai.iterrows():
        print(f"   - {r['name']} (cik={r['cik']}): AI {int(r['patents_ai'])} / Total {int(r['patents_total'])}")
