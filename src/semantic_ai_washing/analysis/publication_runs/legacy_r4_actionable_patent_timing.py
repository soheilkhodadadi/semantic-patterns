"""Publication run driver for legacy Table R4: actionable disclosure and AI patent timing."""
...
DEFAULT_TEST_ROOT = Path(
    "/Users/soheilkhodadadi/DataWork/semantic-patterns/ai_washing/derived/test_runs/legacy_r4_actionable_patent_timing"
)
DEFAULT_PAPER_ROOT = REPO_ROOT / "paper/generated"
DEFAULT_RUN_ID = f"{date.today():%Y%m%d}_hybrid_api_a_conf49_main_v1"
TEST_ID = "legacy_r4_actionable_patent_timing"
MODULE_PATH = "semantic_ai_washing.analysis.publication_runs.legacy_r4_actionable_patent_timing"
...
def _prepare_payload(panel_path: Path) -> dict[str, object]:
    payload = summarize_table_4_actionable_patent_timing(panel_path)
    payload["title"] = "Table R4. Actionable Disclosure and AI Patent Timing"
...
def _writer_packet(args: argparse.Namespace, payload: dict[str, object]) -> str:
    return "\n".join(
        [
            ...
            "",
            "## Caption Draft",
            "This table reports distributed-lag style regressions of actionable disclosure on AI patent timing in the expanded 2016-2025 ever-speaker panel. Each column varies the fixed-effects structure or sample trim, while the rows trace patent timing from t-2 through t+2. Standard errors are clustered at the firm level.",
            "",
        ]
    )
