"""Publication run driver for legacy Figure R3: patent mismatch alignment by A/S quantile."""
...
from semantic_ai_washing.analysis.build_delivery_figures import _save_figure_docx, build_figure_3
...
DEFAULT_TEST_ROOT = Path(
    "/Users/soheilkhodadadi/DataWork/semantic-patterns/ai_washing/derived/test_runs/legacy_r3_patent_mismatch_alignment"
)
DEFAULT_PAPER_ROOT = REPO_ROOT / "paper/generated"
DEFAULT_RUN_ID = f"{date.today():%Y%m%d}_hybrid_api_a_conf49_main_v1"
TEST_ID = "legacy_r3_patent_mismatch_alignment"
MODULE_PATH = "semantic_ai_washing.analysis.publication_runs.legacy_r3_patent_mismatch_alignment"
...
def main() -> None:
    ...
    image_path, docx_path = build_figure_3(args.annual_panel, run_dir, run_dir)
    normalized_png = run_dir / "figure_main.png"
    shutil.copy2(image_path, normalized_png)
    normalized_docx = run_dir / "figure_main.docx"
    _save_figure_docx(
        "Figure R3. Patent Alignment and Mismatch by A/S Quantile",
        "This figure uses the firm-plus-year fixed-effect mismatch specification from Tables R6 and R7. The x-axis groups AI-talking firm-years into pooled quartiles of the A/S ratio, using the quartile medians as reference points. The two lines plot the model-implied relation between A/S and future AI patenting for mismatch and non-mismatch firm-years.",
        normalized_png,
        normalized_docx,
    )
    (run_dir / "result_notes.md").write_text(
        "# Result Notes\n\n- This figure traces the predicted within-sample relation between the A/S ratio and future AI patenting separately for mismatch and non-mismatch firm-years.\n- It is anchored on the same methodology-aligned mismatch specification used in Tables R6 and R7.\n",
        encoding="utf-8",
    )
    (run_dir / "writer_packet.md").write_text(
        "\n".join(
            [
                ...
                "",
                "## Caption Draft",
                "This figure uses the firm-plus-year fixed-effect mismatch specification from Tables R6 and R7. The x-axis groups AI-talking firm-years into pooled quartiles of the A/S ratio, and the lines show the implied relation between A/S and future AI patenting for mismatch and non-mismatch firm-years.",
                "",
            ]
        ),
        encoding="utf-8",
