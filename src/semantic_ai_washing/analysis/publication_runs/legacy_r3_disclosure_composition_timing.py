"""Publication run driver for legacy Table R3: disclosure composition and AI patent timing."""
...
DEFAULT_TEST_ROOT = Path(
    "/Users/soheilkhodadadi/DataWork/semantic-patterns/ai_washing/derived/test_runs/legacy_r3_disclosure_composition_timing"
)
DEFAULT_PAPER_ROOT = REPO_ROOT / "paper/generated"
DEFAULT_RUN_ID = f"{date.today():%Y%m%d}_hybrid_api_a_conf49_main_v1"
TEST_ID = "legacy_r3_disclosure_composition_timing"
MODULE_PATH = (
    "semantic_ai_washing.analysis.publication_runs.legacy_r3_disclosure_composition_timing"
)
...
def _prepare_payload(panel_path: Path) -> dict[str, object]:
    payload = summarize_table_3_timing_composition(panel_path)
    payload["title"] = "Table R3. Disclosure Composition and AI Patent Timing"
...
def _result_notes(payload: dict[str, object]) -> str:
    ...
    panels = payload["panels"]  # type: ignore[assignment]
    act = {
        model["label"]: coef for model, coef in zip(models, panels[0]["coef_cells"], strict=True)
    }
    spec = {
        model["label"]: coef for model, coef in zip(models, panels[1]["coef_cells"], strict=True)
    }
    footer_lookup = {row["label"]: row["cells"] for row in panels[1]["footer_rows"]}  # type: ignore[index]
    return "\n".join(
        [
            "# Result Notes",
            "",
            f"- Actionable disclosure coefficient at `t`: `{act['t']}` and at `t+1`: `{act['t+1']}`.",
            f"- Speculative-only disclosure coefficient at `t`: `{spec['t']}` and at `t+1`: `{spec['t+1']}`.",
            f"- Actionable disclosure remains positive at `t+2`: `{act['t+2']}`.",
...
def _writer_packet(args: argparse.Namespace, payload: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "## Main coefficients",
            f"- Actionable disclosure at `t`: `{act['t']}`",
            f"- Actionable disclosure at `t+1`: `{act['t+1']}`",
            f"- Actionable disclosure at `t+2`: `{act['t+2']}`",
            f"- Speculative-only disclosure at `t`: `{spec['t']}`",
            f"- Speculative-only disclosure at `t+1`: `{spec['t+1']}`",
            f"- Speculative-only disclosure at `t+2`: `{spec['t+2']}`",
            "",
            "## Caption Draft",
            "This table reports firm-year panel regressions of AI patent timing on disclosure composition in the expanded 2016-2025 ever-speaker panel. Panel A uses the actionable-disclosure indicator and Panel B uses the speculative-only disclosure indicator. Columns vary the patent horizon from t-2 through t+2 and absorb firm and year fixed effects with firm-clustered standard errors.",
            "",
        ]
    )
