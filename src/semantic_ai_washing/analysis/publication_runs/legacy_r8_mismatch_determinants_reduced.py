"""Publication run driver for legacy Table R8: reduced-baseline determinants of PatentMismatch."""
...
DEFAULT_TEST_ROOT = Path(
    "/Users/soheilkhodadadi/DataWork/semantic-patterns/ai_washing/derived/test_runs/legacy_r8_mismatch_determinants_reduced"
)
DEFAULT_PAPER_ROOT = REPO_ROOT / "paper/generated"
DEFAULT_RUN_ID = f"{date.today():%Y%m%d}_hybrid_api_a_conf49_main_v1"
TEST_ID = "legacy_r8_mismatch_determinants_reduced"
MODULE_PATH = (
    "semantic_ai_washing.analysis.publication_runs.legacy_r8_mismatch_determinants_reduced"
)
...
def _prepare_payload(panel_path: Path) -> dict[str, object]:
    payload = summarize_table_7c_mismatch_determinants_reduced(panel_path)
    payload["title"] = "Table R8. Reduced-Baseline Determinants of PatentMismatch"
    payload["dependent_label"] = "Dependent variable: any PatentMismatch incident, 2016-2025"
    payload["note"] = (
        str(payload["note"])
        .replace("2016-2024", "2016-2025")
        .replace("This refinement revisits", "This table updates")
    )
    return payload
...
def _result_notes(payload: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            "# Result Notes",
            "",
            f"- In the multivariate reduced-baseline column, `Log assets` is `{lookup['Log assets']}`.",
            f"- In the multivariate reduced-baseline column, `ROA` is `{lookup['ROA']}`.",
            f"- In the multivariate reduced-baseline column, `Leverage` is `{lookup['Leverage']}`.",
            f"- The reduced multivariate sample keeps `{footer['Observations'][multivar_idx]}` firms, materially more than the full-baseline determinant design.",
            "",
        ]
    )
...
def _writer_packet(args: argparse.Namespace, payload: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            ...
            "- Key regressors: `log assets, cash/assets, leverage, CAPX/assets, ROA`",
            "- Control philosophy: `reduced baseline set to preserve complete-case sample relative to the full determinant table`",
            "",
            "## Estimation Block",
            ...
            "- One-sentence interpretation: `on the larger reduced-baseline sample, firm size is the only clearly stable reduced-baseline predictor of ever recording a mismatch incident`",
            "- Candidate use: `appendix table with possible citation in main text when discussing mismatch composition`",
            "",
            "## Caption Draft",
            "This table reports reduced-baseline cross-sectional regressions for whether a firm records at least one PatentMismatch incident on the expanded 2016-2025 panel. Baseline covariates are measured in 2016 and include log assets, cash/assets, leverage, CAPX/assets, and ROA. All specifications include baseline industry fixed effects, and standard errors are clustered at the industry level.",
            "",
        ]
    )
