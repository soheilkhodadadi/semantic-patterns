"""Publication run driver for Test 03: post-filing drift."""
...
DEFAULT_TEST_ROOT = Path(
    "/Users/soheilkhodadadi/DataWork/semantic-patterns/ai_washing/derived/test_runs/test_03_post_filing_drift"
)
DEFAULT_PAPER_ROOT = REPO_ROOT / "paper/generated"
DEFAULT_RUN_ID = f"{date.today():%Y%m%d}_hybrid_api_a_conf49_main_v1"
TEST_ID = "test_03_post_filing_drift"
MODULE_PATH = "semantic_ai_washing.analysis.publication_runs.test_03_post_filing_drift"
CONTROL_TERMS = ["ln_assets", "leverage", "cash", "roa"]
FOCAL_TERMS = ["PatentMismatch", "A_S", "AI_Focus"]
SUPPORT_FIGURE_DAY_RANGE = (2, 252)
...
REGRESSION_SPECS = [
    RegressionSpec(
        model_id="post_bhar_1m_cluster",
        dependent="post_bhar_1m",
        title="BHAR[+2,+21]",
        start_day=2,
        end_day=21,
    ),
    RegressionSpec(
        model_id="post_bhar_3m_cluster",
        dependent="post_bhar_3m",
        title="BHAR[+2,+63]",
        start_day=2,
        end_day=63,
    ),
    RegressionSpec(
        model_id="post_bhar_6m_cluster",
        dependent="post_bhar_6m",
        title="BHAR[+2,+126]",
        start_day=2,
        end_day=126,
    ),
    RegressionSpec(
        model_id="post_bhar_12m_cluster",
        dependent="post_bhar_12m",
        title="BHAR[+2,+252]",
        start_day=2,
        end_day=252,
    ),
...
def _window_output_name(spec: RegressionSpec) -> str:
    suffix = spec.title.replace("[", "_").replace("]", "").replace(",", "_").replace("+", "p")
    return suffix.replace("-", "m")


def _compute_post_bhar_table(daily_returns: Path) -> pd.DataFrame:
    daily = pd.read_parquet(
        daily_returns,
        columns=["filing_id", "relative_day", "ret", "vwretd", "abnormal_ret_vw"],
    ).copy()
    daily["relative_day"] = pd.to_numeric(daily["relative_day"], errors="coerce").astype(int)
    for column in ["ret", "vwretd", "abnormal_ret_vw"]:
        daily[column] = pd.to_numeric(daily[column], errors="coerce")

    rows: list[dict[str, object]] = []
    for filing_id, group in daily.groupby("filing_id", sort=False):
        ordered = group.sort_values("relative_day").reset_index(drop=True)
        observed_days = set(ordered["relative_day"].tolist())
        row: dict[str, object] = {"filing_id": filing_id}
        for spec in REGRESSION_SPECS:
            needed_days = set(range(spec.start_day, spec.end_day + 1))
            name = spec.dependent
            complete = needed_days.issubset(observed_days)
            sub = ordered.loc[ordered["relative_day"].between(spec.start_day, spec.end_day)].copy()
            if complete and sub["ret"].notna().all() and sub["vwretd"].notna().all():
                firm_bh = (1.0 + sub["ret"]).prod() - 1.0
                market_bh = (1.0 + sub["vwretd"]).prod() - 1.0
                row[name] = float(firm_bh - market_bh)
                row[f"{name}_complete"] = True
            else:
                row[name] = None
                row[f"{name}_complete"] = False
        rows.append(row)
...
def _drift_path_summary(sample: pd.DataFrame, daily_returns: Path) -> pd.DataFrame:
    keep_ids = sample.loc[
        sample["is_ai_filing"] & sample["post_bhar_12m"].notna(),
        ["filing_id", "PatentMismatch"],
    ].copy()
    ...
    daily = daily.loc[
        daily["relative_day"].between(SUPPORT_FIGURE_DAY_RANGE[0], SUPPORT_FIGURE_DAY_RANGE[1])
    ].copy()
    daily["abnormal_ret_vw"] = pd.to_numeric(daily["abnormal_ret_vw"], errors="coerce")
...
def _plot_drift_figure(summary: pd.DataFrame, output_base: Path) -> tuple[Path, Path]:
    _base_style()
    ...
    ax.set_xlabel("Trading days after filing (+2 onward)")
    ax.set_ylabel("Cumulative mean abnormal return (pct)")
    ax.set_title("Post-Filing Drift After Annual 10-K Release")
...
def _render_table_outputs(
    ...
    panel_b_rows.append(["Controls"] + ["Reduced baseline"] * len(regression_rows))
    ...
    md = "\n".join(
        [
            ...
            "",
            "## Panel B. Post-filing drift regressions",
    ...
    latex_lines = [
        "\\begin{table}[!htbp]",
        "\\centering",
        "\\caption{Post-filing abnormal returns and PatentMismatch}",
...
def _build_panel_table(
    ...
    widths = [Inches(2.8)] + [Inches(1.55)] * (len(headers) - 1)
...
def _build_table_docx(
    ...
    _add_title(document, "Table 2. Post-Filing Drift Following Annual 10-K Releases")
    note = (
        "This table reports post-filing buy-and-hold abnormal returns (BHAR) after annual 10-K releases, using market-adjusted daily returns and excluding the immediate filing window by starting at trading day +2. "
        "Panel A reports mean BHARs by PatentMismatch status and the mismatch-minus-non-mismatch difference. "
        "Panel B reports filing-level regressions of post-filing BHAR on PatentMismatch, the A/S ratio, and AI Focus with a reduced lagged control set, industry fixed effects, filing-year fixed effects, and standard errors clustered by firm. "
    ...
    run = panel_b.add_run("Panel B. Post-filing drift regressions")
    _set_run_font(run, size=11, bold=True)
    ...
    panel_b_rows.extend(
        [
            [("Controls", True)] + [("Reduced baseline", False) for _ in windows],
...
def _result_notes(
    ...
    short_row = next(row for row in diff_rows if row["window_label"] == "BHAR[+2,+21]")
    long_row = next(row for row in diff_rows if row["window_label"] == "BHAR[+2,+252]")
    long_reg = next(row for row in regression_rows if row["dependent"] == "post_bhar_12m")
    mismatch_term = long_reg["focal_terms"]["PatentMismatch"]
    return "\n".join(
        [
            "# Result Notes",
            "",
            f"- BHAR[+2,+21] mean difference (mismatch minus non-mismatch): `{100 * short_row['diff_mismatch_minus_non']:.3f}` percentage points, p=`{short_row['p_value']:.3f}`.",
            f"- BHAR[+2,+252] mean difference (mismatch minus non-mismatch): `{100 * long_row['diff_mismatch_minus_non']:.3f}` percentage points, p=`{long_row['p_value']:.3f}`.",
            f"- Regression coefficient on `PatentMismatch` in BHAR[+2,+252]: `{mismatch_term['coef']:.4f}` with p=`{mismatch_term['p_value']:.3f}`.",
            "- The supporting figure traces cumulative mean abnormal returns from trading day +2 to +252 using only filings with a complete 12-month window.",
            "",
        ]
    )
...
def _writer_packet(
    ...
    return "\n".join(
        [
            ...
            "- Sample filters: `AI-talking 10-K / 10-K-A filings with complete post-filing window, annual PatentMismatch merge, reduced lagged controls, industry FE, and filing-year FE`",
            "- Date range: `2016-2024`",
            f"- Exact N: `{int(sample['is_ai_filing'].sum()):,}` AI filings in the event sample; `BHAR[+2,+252]` regression N = `{long_reg['nobs']:,}`",
            "",
            "## Variables",
            "- Dependent variable: `market-adjusted post-filing buy-and-hold abnormal return starting at trading day +2`",
            ...
            "- Control set: `ln_assets`, `leverage`, `cash`, `roa` (reduced baseline to preserve long-horizon coverage)",
            "- Transformations: `industry FE via SIC2`, `filing-year FE`, `clustered standard errors at gvkey`",
            "",
            "## Estimation",
            "- Model equation: `PostBHAR ~ PatentMismatch + A_S + AI_Focus + controls + Industry FE + Filing-year FE`",
            "- Fixed effects: `sic2` and `filing_year`",
            "- Clustering: `gvkey`",
            "- Weighting: `none`",
            "- Benchmark / abnormal return model: `buy-and-hold firm return minus buy-and-hold CRSP value-weighted market return`",
            "",
            "## Results",
            f"- Key coefficient(s) or spread(s): `BHAR[+2,+252]` mismatch-minus-non-mismatch mean difference = `{100 * long_row['diff_mismatch_minus_non']:.3f}` pp; regression `PatentMismatch` coefficient = `{mismatch_term['coef']:.4f}`",
            f"- Standard error / t-stat / p-value: `PatentMismatch` SE = `{mismatch_term['se']:.4f}`, p = `{mismatch_term['p_value']:.3f}`",
            "- Economic magnitude: `Interpret post-filing BHAR coefficients as percentage-point shifts in abnormal performance after the initial filing window has passed.`",
            "- One-sentence interpretation: `This test asks whether mismatch-labeled AI disclosure is followed by medium-horizon price correction after the filing-date window closes.`",
            "- Main text / appendix / discard: `main text candidate if the horizon pattern is monotone or strongest at longer windows`",
            "",
            "## Caption Draft",
            "This table reports post-filing buy-and-hold abnormal returns after annual 10-K releases, starting at trading day +2 so the immediate filing-date window is excluded. Panel A reports mean post-filing BHARs by PatentMismatch status and the mismatch-minus-non-mismatch difference. Panel B reports filing-level regressions of BHAR on PatentMismatch, the A/S ratio, and AI Focus with lagged controls, industry fixed effects, filing-year fixed effects, and standard errors clustered by firm. A supporting figure traces cumulative mean abnormal returns through trading day +252 on the complete 12-month window sample.",
            "",
            "## Notes",
            "- Attrition / missingness note: `Longer horizons shrink the usable filing sample because a complete post-filing return window is required through the horizon endpoint.`",
            "- Any unusual diagnostics: `The main table uses windows built directly from the daily event-return file, not the earlier placeholder BHAR columns, so the drift horizons cleanly begin at trading day +2.`",
            "",
        ]
    )
...
def _dataset_summary(
    ...
    *,
    ...
    completion = {
        spec.dependent: int(sample[spec.dependent].notna().sum()) for spec in REGRESSION_SPECS
    }
    return {
        "test_id": TEST_ID,
        "run_id": args.run_id,
        ...
        "post_window_counts": completion,
        "difference_in_means": diff_rows,
        "regressions": regression_rows,
        "drift_path_days": drift_summary.to_dict(orient="records"),
    }
...
def _copy_exports(run_dir: Path, paper_root: Path, run_id: str) -> dict[str, str]:
    exports = {
        ...
        "drift_path_summary": paper_root / "tables" / f"{TEST_ID}_{run_id}_drift_path_summary.csv",
    ...
    mapping = {
        ...
        run_dir / "drift_path_summary.csv": exports["drift_path_summary"],
...
def main() -> None:
    ...
    post_bhar = _compute_post_bhar_table(args.daily_returns)
    sample = sample.merge(post_bhar, on="filing_id", how="left", validate="one_to_one")

    diff_rows = []
    for spec in REGRESSION_SPECS:
        result = _difference_in_means(sample, spec.dependent)
        result["window_label"] = spec.title
        result["start_day"] = spec.start_day
        result["end_day"] = spec.end_day
        diff_rows.append(result)

    regression_rows = [_fit_regression(sample, spec) for spec in REGRESSION_SPECS]
    drift_summary = _drift_path_summary(sample, args.daily_returns)
        ...
        table_rows.append(
            {
                ...
                "mean_mismatch": row["mean_mismatch"],
                "mean_non_mismatch": row["mean_non_mismatch"],
    ...
    _build_table_docx(diff_rows, regression_rows, run_dir / "table_main.docx")
    drift_summary.to_csv(run_dir / "drift_path_summary.csv", index=False)
    png_path, pdf_path = _plot_drift_figure(drift_summary, run_dir / "figure_main")

    (run_dir / "result_notes.md").write_text(
        _result_notes(diff_rows, regression_rows), encoding="utf-8"
    )
    (run_dir / "writer_packet.md").write_text(
        _writer_packet(args, sample, diff_rows, regression_rows), encoding="utf-8"
    )
    ...
    manifest = {
        ...
        "outputs": {
            ...
            "drift_path_summary": str(run_dir / "drift_path_summary.csv"),
