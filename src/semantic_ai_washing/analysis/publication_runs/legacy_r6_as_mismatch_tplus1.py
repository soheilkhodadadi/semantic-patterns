"""Publication run driver for legacy Table R6: A/S ratio x PatentMismatch at t+1."""
...
DEFAULT_TEST_ROOT = Path(
    "/Users/soheilkhodadadi/DataWork/semantic-patterns/ai_washing/derived/test_runs/legacy_r6_as_mismatch_tplus1"
)
DEFAULT_PAPER_ROOT = REPO_ROOT / "paper/generated"
DEFAULT_RUN_ID = f"{date.today():%Y%m%d}_hybrid_api_a_conf49_main_v1"
TEST_ID = "legacy_r6_as_mismatch_tplus1"
MODULE_PATH = "semantic_ai_washing.analysis.publication_runs.legacy_r6_as_mismatch_tplus1"
...
def _prepare_payload(panel_path: Path) -> dict[str, object]:
    payload = summarize_table_6_as_patent_mismatch_tplus1(panel_path)
    payload["title"] = "Table R6. A/S Ratio, Patent Mismatch, and Future AI Patenting"
    payload["dependent_label"] = "Dependent variable: log(1 + AI patents at t+1)"
...
def _result_notes(payload: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            "# Result Notes",
            "",
            f"- Under `{first_spec}`, `A/S ratio` at `t+1` is `{lookup['A/S ratio']}`.",
            f"- Under `{first_spec}`, `A/S ratio × PatentMismatch` at `t+1` is `{lookup['A/S ratio × PatentMismatch']}`.",
            f"- Observation counts range from `{footer['Observations'][0]}` to `{footer['Observations'][-1]}` across specifications.",
            "",
        ]
    )
...
def _writer_packet(args: argparse.Namespace, payload: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            ...
            "- Date range: `2016-2025 disclosures with patent lead outcome at t+1`",
            ...
            "",
            "## Result Block",
            f"- First-spec `A/S ratio`: `{lookup['A/S ratio']}`",
            f"- First-spec `A/S ratio × PatentMismatch`: `{lookup['A/S ratio × PatentMismatch']}`",
            "- One-sentence interpretation: `higher actionable-to-speculative disclosure predicts next-year AI patenting only when the disclosure is not flagged as mismatch`",
            "- Candidate use: `main text validation table`",
            "",
            "## Caption Draft",
            "This table reports the methodology-aligned AI-washing specification on the expanded 2016-2025 ever-speaker annual panel. The dependent variable is log(1 + AI patents) at t+1. Rows report the A/S ratio and its interaction with PatentMismatch, while columns vary the fixed-effects structure and sample trim. Controls are lagged firm characteristics, and standard errors are clustered at the firm level.",
            "",
        ]
    )
