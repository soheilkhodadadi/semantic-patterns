"""Publication run driver for legacy Table R7: A/S ratio x PatentMismatch at t+2."""
...
DEFAULT_TEST_ROOT = Path(
    "/Users/soheilkhodadadi/DataWork/semantic-patterns/ai_washing/derived/test_runs/legacy_r7_as_mismatch_tplus2"
)
DEFAULT_PAPER_ROOT = REPO_ROOT / "paper/generated"
DEFAULT_RUN_ID = f"{date.today():%Y%m%d}_hybrid_api_a_conf49_main_v1"
TEST_ID = "legacy_r7_as_mismatch_tplus2"
MODULE_PATH = "semantic_ai_washing.analysis.publication_runs.legacy_r7_as_mismatch_tplus2"
...
def _prepare_payload(panel_path: Path) -> dict[str, object]:
    payload = summarize_table_6b_as_patent_mismatch_tplus2(panel_path)
    payload["title"] = "Table R7. A/S Ratio, Patent Mismatch, and Longer-Horizon AI Patenting"
    payload["dependent_label"] = "Dependent variable: log(1 + AI patents at t+2)"
...
def _result_notes(payload: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            "# Result Notes",
            "",
            f"- Under `{first_spec}`, `A/S ratio` at `t+2` is `{lookup['A/S ratio']}`.",
            f"- Under `{first_spec}`, `A/S ratio × PatentMismatch` at `t+2` is `{lookup['A/S ratio × PatentMismatch']}`.",
