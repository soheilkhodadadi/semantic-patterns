...
class DirectorOptimizer:
    ...
    def optimize(self, focus_iteration: str = "", focus_phase: str = "") -> OptimizationReport:
        ...
        task_states = evaluator.evaluate_all()
        ...
        ready = sorted(
            [state for state in scoped_states if state.status == "ready"],
        ...
        patch = self._patch_proposal(
            ...
            blocked_state=blocked[0] if blocked else None,
        ...
        recommended_task_ids = [item.task_id for item in ready[:3]]
        if patch is not None:
        ...
        recommendation = OptimizationRecommendation(
            ...
            blocked_task_ids=[item.task_id for item in blocked[:3]],
            rationale=[
                f"Top ready task `{item.task_id}` has score {item.score:.1f}."
                for item in ready[:3]
            ]
            + [f"Blocked task `{item.task_id}` remains `{item.status}`." for item in blocked[:3]]
        ...
        dump_json(
            readiness_file,
            {
                ...
                "task_states": [item.as_deterministic_dict() for item in scoped_states],
            },
        )
        ...
        recommendation_markdown.write_text(
            render_optimization_markdown(
                ...
                task_state_rows=[item.as_deterministic_dict() for item in scoped_states],
            ),
            encoding="utf-8",
        )
