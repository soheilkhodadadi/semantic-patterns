...
def load_company_list(path="data/metadata/company_list_50.csv") -> pd.DataFrame:
    ...
    name_col = next(
        (
            cols[k]
            for k in ["name", "company", "company_name", "issuer", "issuer_name", "conm"]
            if k in cols
        ),
        None,
    )
...
def connect_wrds():
    ...
    conn = psycopg2.connect(dbname="wrds", user=user, password=pwd, host=host, port=port)
    return conn
...
def build_cik_gvkey_crosswalk(conn, companies: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        print(
            "[!] 'tic' not found in comp.company (or SELECT failed). Falling back to CIK/name only."
        )
        sql = dedent("""
            SELECT gvkey,
                   CASE WHEN cik IS NULL THEN NULL ELSE cik::text END AS cik,
    ...
    comp_keep["match_key"] = np.where(
        comp_keep["cik"].isin(ciks),
        "cik",
        np.where(comp_keep["ticker_comp"].isin(tics), "ticker", "name"),
    )
    ...
    if "ticker_comp" in comp_keep.columns and comp_keep["ticker_comp"].notna().any():
        have_tic = comp_keep[
            (comp_keep["match_key"] == "ticker")
            & (~comp_keep["ticker_comp"].isin(have_cik["ticker_comp"]))
        ]  # may be empty
    else:
    ...
    cross = pd.concat(
        [
            have_cik.drop_duplicates(subset=["cik"]),
            have_tic.drop_duplicates(subset=["ticker_comp"]),
        ],
        ignore_index=True,
    )
...
def compute_controls(funda: pd.DataFrame, align: str = "fyear") -> pd.DataFrame:
    ...
    funda = funda.sort_values(["gvkey", "year", "datadate"]).drop_duplicates(
...
def write_qc_report(
    path_md: str, cross: pd.DataFrame, controls: pd.DataFrame, start_year: int, end_year: int
):
    lines = []
    ...
    if not controls.empty:
        ...
        miss_tbl = (
            miss_rate.to_frame("missing_share").reset_index().rename(columns={"index": "column"})
        )
...
def main():
    ...
    controls = controls.merge(
        cross[["cik", "gvkey", "sic"]].drop_duplicates(), on="gvkey", how="left"
    )
    # Zero-pad CIK
