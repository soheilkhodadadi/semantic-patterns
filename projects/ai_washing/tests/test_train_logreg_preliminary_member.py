...
from ai_washing_member.classification.train_logreg_preliminary import run_training


def test_member_train_logreg_preliminary_smoke(tmp_path):
    ...
    metadata = run_training(
        argparse.Namespace(
            ...
            model_output=str(tmp_path / "artifacts" / "model.pkl"),
            metadata_output=str(tmp_path / "artifacts" / "metadata.json"),
            model_id="member-logreg-test",
    ...
    assert metadata["model_type"] == "logreg_multiclass"
